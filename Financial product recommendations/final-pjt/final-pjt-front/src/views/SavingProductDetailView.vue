...
axios({
    method: 'get',
    url: `${store.API_URL}/products/saving-products/options/${route.params.str}/`,
    headers : {
...
onMounted(() => {
    axios({
        method : 'get',
        url : `${store.API_URL}/products/saving-products/detail/${route.params.str}/`,
        headers : {
            Authorization: `Token ${store.token}`
        }
    }).then((response) => {
...
const productSignup = () => {
    axios({
        method : 'get',
        url : `${store.API_URL}/products/saving-products/join/${route.params.str}/`,
        headers : {
            Authorization: `Token ${store.token}`
        }
    })
...
const productSignout = () => {
    axios({
        method : 'get',
        url : `${store.API_URL}/products/saving-products/remove/${route.params.str}/`,
        headers : {
            Authorization: `Token ${store.token}`
        }
})}
...
const isProductSignedUp = computed(() => {
    return savingproducts.value.some(item => item.fin_prdt_nm === product.value.fin_prdt_nm);
});
